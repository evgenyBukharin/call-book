<template>
    ...
    >
        <div class="custom-select__container-text">
            <span class="custom-select__value custom-select__value-default">{{ $store.state.selectedFilter.name }}</span>
            <span class="custom-select__value custom-select__value-hover">{{ $store.state.selectedFilter.name }}</span>
        </div>
        ...
        <ul class="list-reset custom-select__list" ref="customSelectList">
            <li
                v-for="(filter, idx) in $store.state.filtersList"
                :key="'filter' + idx"
                class="custom-select__item"
                @click="filterChangeHandler(filter)"
            >
                <span>{{ filter.name }}</span>
                <img
                    src="../assets/img/okay.svg"
                    class="image custom-select__icon"
                    width="16"
                    height="16"
                    alt="Галочка"
                    v-if="filter.name == $store.state.selectedFilter.name"
                />
            </li>
        </ul>
    </div>
...
export default {
    ...
    methods: {
        ...
        filterChangeHandler(newFilter) {
            this.$store.commit('updateSelectedFilter', newFilter);
            if (newFilter.id !== 0 && newFilter.name !== 'Все') {
                this.$store.commit('updateFilteredContacts', this.filterContacts());
            } else {
                this.$store.commit('updateFilteredContacts', this.$store.state.contacts);
            }
        },
        filterContacts() {
            return this.$store.state.contacts.filter((filter) => {
                return filter.group == this.$store.state.selectedFilter.name;
            });
        },
    },
};
